...
tasks.register("generateConstants") {
  ...
  val file = file(constantsFile)
  val licenseLines = layout.projectDirectory.file("LICENSE").asFile.readLines().map { it.trim() }
...
tasks.register("checkGameControllerDBSubmodule") {
  ...
  val file = file(gamecontrollerdbGitFile)
  doLast {
    if (!file.exists()) {
...
tasks.register("customizeLoggingProperties") {
  ...
  dependsOn("jlink")
  val file = runtimeDir.get().file("conf/logging.properties")
  val projectName = project.name
  doLast {
    ant.withGroovyBuilder {
      "propertyfile"("file" to file) {
        "entry"(
            "key" to "handlers",
