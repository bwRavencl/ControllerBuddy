...
/**
 * Utility class that allows for self-referential resource bundles -- i.e.,
 * ResourceBundles with messages that can reference keys from the same bundle
 * using standard JSP-inline ${} notation. Each instance of the class maintains
 * a simple cache of translated bundles for additional lookup performance. To
 * avoid potential infinite recursion, the maximum recursion depth is set to 5.
 * <p/>
 * Example: my.bundle.user.name = Bob Smith my.bundle.user.state = Maine
 * my.bundle.greeting = Hello, ${my.bundle.user.name}, I see you're from
 * ${my.bundle.user.state}.
 */
	...
	public ResourceBundle getResourceBundle(String baseName, Locale locale) {
		...
		if (bundle == null) {
			...
			while (keys.hasMoreElements()) {
				...
				messages.put(key, translateMessage(fetchedBundle, key)); // figure
																			// out
																			// any
																			// inline
																			// refs
