                ...
                try (final var byteArrayOutputStream = new ByteArrayOutputStream();
                    ...
                    do {
                        ...
                        try {
                                    ...
                                    if (serverProtocolVersion != ServerRunMode.PROTOCOL_VERSION) {
                                        ...
                                        EventQueue.invokeLater(() -> GuiUtils.showMessageDialog(
                                                ...
                                                MessageFormat.format(
                                                        ...
                                                        ServerRunMode.PROTOCOL_VERSION,
                            ...
                            final var finalRetry = retry;
                            EventQueue.invokeLater(() -> main.setStatusBarText(MessageFormat.format(
                                    Main.strings.getString("STATUS_TIMEOUT_RETRYING"),
                                    N_CONNECTION_RETRIES - finalRetry,
                                    N_CONNECTION_RETRIES)));
                        }
                    ...
                    if (success) {
                        ...
                        EventQueue.invokeLater(() -> main.setStatusBarText(MessageFormat.format(
                                Main.strings.getString("STATUS_CONNECTED_TO"), host, port, pollInterval)));
                    } else {
                    ...
                    try (final var byteArrayInputStream = new ByteArrayInputStream(receivePacket.getData());
                        ...
                        if (messageType == MessageType.Update || messageType == MessageType.UpdateRequestAlive) {
                            ...
                            if (newCounter > counter) {
                                ...
                                updateOutputSets(
                                        (Set<Integer>) objectInputStream.readObject(),
                                        oldDownMouseButtons,
                                        newUpMouseButtons,
                                        newDownMouseButtons,
                                        false);
                                ...
                                final var inputDownModifiers = downKeyStrokes.stream()
                                        .map(KeyStroke::getModifierCodes)
                                        .flatMap(Stream::of)
                                        .collect(Collectors.toSet());
                                updateOutputSets(
                                        inputDownModifiers, oldDownModifiers, newUpModifiers, newDownModifiers, false);

                                final var inputDownNormalKeys = downKeyStrokes.stream()
                                        .map(KeyStroke::getKeyCodes)
                                        .flatMap(Stream::of)
                                        .collect(Collectors.toSet());
                                updateOutputSets(
                                        inputDownNormalKeys,
                                ...
                                ((Set<Integer>) objectInputStream.readObject())
                                        .stream()
                                                .map(LockKey.virtualKeyCodeToLockKeyMap::get)
