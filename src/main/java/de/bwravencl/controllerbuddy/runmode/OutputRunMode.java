...
public abstract class OutputRunMode extends RunMode {
    ...
    private static final String VJOY_UNINSTALL_REGISTRY_KEY =
    ...
    private static final EventCode[] UINPUT_JOYSTICK_BUTTON_EVENT_CODES = {
        EventCode.BTN_TRIGGER,
        EventCode.BTN_THUMB,
        EventCode.BTN_THUMB2,
        EventCode.BTN_TOP,
        EventCode.BTN_TOP2,
        EventCode.BTN_PINKIE,
        EventCode.BTN_BASE,
        EventCode.BTN_BASE2,
        EventCode.BTN_BASE3,
        EventCode.BTN_BASE4,
        ...
        EventCode.BTN_BASE6,
        EventCode.BTN_DEAD,
        EventCode.BTN_TRIGGER_HAPPY1,
        EventCode.BTN_TRIGGER_HAPPY2,
        EventCode.BTN_TRIGGER_HAPPY3,
        EventCode.BTN_TRIGGER_HAPPY4,
        EventCode.BTN_TRIGGER_HAPPY5,
        EventCode.BTN_TRIGGER_HAPPY6,
        EventCode.BTN_TRIGGER_HAPPY7,
        EventCode.BTN_TRIGGER_HAPPY8,
        EventCode.BTN_TRIGGER_HAPPY9,
        EventCode.BTN_TRIGGER_HAPPY10,
        EventCode.BTN_TRIGGER_HAPPY11,
        EventCode.BTN_TRIGGER_HAPPY12,
        EventCode.BTN_TRIGGER_HAPPY13,
        EventCode.BTN_TRIGGER_HAPPY14,
        EventCode.BTN_TRIGGER_HAPPY15,
        EventCode.BTN_TRIGGER_HAPPY16,
        EventCode.BTN_TRIGGER_HAPPY17,
        EventCode.BTN_TRIGGER_HAPPY18,
        EventCode.BTN_TRIGGER_HAPPY19,
        EventCode.BTN_TRIGGER_HAPPY20,
        EventCode.BTN_TRIGGER_HAPPY21,
        EventCode.BTN_TRIGGER_HAPPY22,
        EventCode.BTN_TRIGGER_HAPPY23,
        EventCode.BTN_TRIGGER_HAPPY24,
        EventCode.BTN_TRIGGER_HAPPY25,
        EventCode.BTN_TRIGGER_HAPPY26,
        EventCode.BTN_TRIGGER_HAPPY27,
        EventCode.BTN_TRIGGER_HAPPY28,
        EventCode.BTN_TRIGGER_HAPPY29,
        EventCode.BTN_TRIGGER_HAPPY30,
        EventCode.BTN_TRIGGER_HAPPY31,
        EventCode.BTN_TRIGGER_HAPPY32,
        EventCode.BTN_TRIGGER_HAPPY33,
        EventCode.BTN_TRIGGER_HAPPY34,
        EventCode.BTN_TRIGGER_HAPPY35,
        EventCode.BTN_TRIGGER_HAPPY36,
        EventCode.BTN_TRIGGER_HAPPY37,
        EventCode.BTN_TRIGGER_HAPPY38,
        EventCode.BTN_TRIGGER_HAPPY39,
        EventCode.BTN_TRIGGER_HAPPY40
    };
    ...
    public static String getDefaultVJoyPath() {
        try {
            return Advapi32Util.registryGetStringValue(
                    WinReg.HKEY_LOCAL_MACHINE, VJOY_UNINSTALL_REGISTRY_KEY, VJOY_INSTALL_LOCATION_REGISTRY_VALUE);
    ...
    private static void setLockKeyState(final LockKey lockKey, final boolean on) {
        if (Main.isWindows) {
            ...
            try {
                ...
                if (state != on) {
                    final var modifierMask =
                            X11WithLockKeyFunctions.INSTANCE.XkbKeysymToModifiers(display, lockKey.keySym());
                    if (modifierMask == 0) {
    ...
    static <T> void updateOutputSets(
            final Set<T> sourceSet,
            final Set<T> oldDownSet,
            final Set<T> newUpSet,
            final Set<T> newDownSet,
            final boolean keepStillDown) {
    ...
    final void deInit() {
        ...
        if (Main.isWindows && vJoy != null) {
            ...
            EventQueue.invokeLater(() -> main.setStatusBarText(MessageFormat.format(
                    Main.strings.getString("STATUS_DISCONNECTED_FROM_VJOY_DEVICE"), vJoyDevice.intValue())));
        } else if (Main.isLinux) {
    ...
    private void doMouseButtonInput(final int button, final boolean down) throws IOException {
        if (Main.isWindows) {
            ...
            switch (button) {
                case 1 -> input.input.mi.dwFlags =
                        down ? new DWORD(MOUSEEVENTF_LEFTDOWN) : new DWORD(MOUSEEVENTF_LEFTUP);
                case 2 -> input.input.mi.dwFlags =
                        down ? new DWORD(MOUSEEVENTF_RIGHTDOWN) : new DWORD(MOUSEEVENTF_RIGHTUP);
                case 3 -> input.input.mi.dwFlags =
                        down ? new DWORD(MOUSEEVENTF_MIDDLEDOWN) : new DWORD(MOUSEEVENTF_MIDDLEUP);
    ...
    final boolean init() {
        ...
        if (Main.isWindows) {
            try {
                ...
                if (!vJoy.DriverMatch(dllVersion, drvVersion).booleanValue()) {
                    ...
                    EventQueue.invokeLater(() -> GuiUtils.showMessageDialog(
                            ...
                            MessageFormat.format(
                                    ...
                                    dllVersion.getValue().shortValue(),
                                    drvVersion.getValue().shortValue(),
                                    Main.strings.getString("ERROR_DIALOG_TITLE"),
                                    JOptionPane.ERROR_MESSAGE)));
                    return false;
                }
                ...
                final var hasAxisX = vJoy.GetVJDAxisExist(vJoyDevice, VjoyInterface.HID_USAGE_X)
                        .booleanValue();
                final var hasAxisY = vJoy.GetVJDAxisExist(vJoyDevice, VjoyInterface.HID_USAGE_Y)
                        .booleanValue();
                final var hasAxisZ = vJoy.GetVJDAxisExist(vJoyDevice, VjoyInterface.HID_USAGE_Z)
                        .booleanValue();
                final var hasAxisRX = vJoy.GetVJDAxisExist(vJoyDevice, VjoyInterface.HID_USAGE_RX)
                        .booleanValue();
                final var hasAxisRY = vJoy.GetVJDAxisExist(vJoyDevice, VjoyInterface.HID_USAGE_RY)
                        .booleanValue();
                final var hasAxisRZ = vJoy.GetVJDAxisExist(vJoyDevice, VjoyInterface.HID_USAGE_RZ)
                        .booleanValue();
                final var hasAxisSL0 = vJoy.GetVJDAxisExist(vJoyDevice, VjoyInterface.HID_USAGE_SL0)
                        .booleanValue();
                final var hasAxisSL1 = vJoy.GetVJDAxisExist(vJoyDevice, VjoyInterface.HID_USAGE_SL1)
                        .booleanValue();
                if (!hasAxisX
                        || !hasAxisY
                        || !hasAxisZ
            ...
            try {
                ...
                joystickInputDevice.addCapability(
                        ...
                        EventCode.ABS_RY,
                        EventCode.ABS_RZ,
                        EventCode.ABS_THROTTLE,
                        EventCode.ABS_RUDDER);
