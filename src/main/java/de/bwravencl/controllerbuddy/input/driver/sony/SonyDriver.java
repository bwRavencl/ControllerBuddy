...
public abstract class SonyDriver extends Driver implements IGamepadStateProvider {
            ...
            try {
                for (; ; ) {
                    ...
                    if (reportId != connection.inputReportId
                            || (bluetooth
                                    ? reportLength < BLUETOOTH_REPORT_LENGTH
    ...
    static HidDevice getHidDevice(
        ...
        try {
            ...
            final var devices = hidServices.getAttachedHidDevices().stream()
                    .filter(hidDevice ->
                            hidDevice.getVendorId() == (short) 0x54C && hidDevice.getProductId() == productId)
                    .toList();

            log.log(
                    Level.INFO,
                    "Found " + devices.size() + " " + humanReadableName + " controller(s): "
            ...
            if (!hidDevice.open()) {
                log.log(
                        Level.WARNING,
                        Main.assembleControllerLoggingMessage(
                                "Could not open HID device " + humanReadableName + " with path " + hidDevice.getPath()
                                        + " to use as",
                                selectedController));
                return null;
            }
    ...
    private static float mapRawAxisToFloat(final byte value) {
        if (value < 0) {
            return Input.normalize(value, -128, -1, 0f, 1f);
        } else {
            return Input.normalize(value, 0, 127, -1f, 0f);
        }
    }
        ...
        if (System.currentTimeMillis() - timestampLastInputReport > INPUT_REPORT_TIMEOUT) {
            getLogger()
                    .log(
                            Level.WARNING,
                            Main.assembleControllerLoggingMessage(
                                    "No new input report for more than " + INPUT_REPORT_TIMEOUT + " ms from",
                                    controller));
    ...
    void updateLightbarColor() {
        ...
        try {
            ...
            if (charging) {
                ...
                hidReport[lightbarOffset + connection.offset] =
                        (byte) (batteryCapacity <= LOW_BATTERY_WARNING ? 0x1C : 0x0);
                hidReport[lightbarOffset + 1 + connection.offset] = 0;
                hidReport[lightbarOffset + 2 + connection.offset] =
                        (byte) (batteryCapacity <= LOW_BATTERY_WARNING ? 0x0 : 0x1C);
            }
