	...
	private void updateTheme() {
		...
		while (keys.hasMoreElements()) {
			...
			if (value instanceof FontUIResource) {
				final var oldFontUIResource = (FontUIResource) value;
				final var newFontUIResource = new FontUIResource(oldFontUIResource.getName(), Font.PLAIN,
						oldFontUIResource.getSize());
		...
		if (usingOceanTheme()) {
			...
			final var panelBackground = UIManager.get("Panel.background");

			if (buttonGradient instanceof List && panelBackground instanceof Color) {
				final var panelBackgroundColor = (Color) panelBackground;
				final var factor = .94f;

				final var buttonGradientColor = new Color(Math.max((int) (panelBackgroundColor.getRed() * factor), 0),
						Math.max((int) (panelBackgroundColor.getGreen() * factor), 0),
						Math.max((int) (panelBackgroundColor.getBlue() * factor), 0), panelBackgroundColor.getAlpha());
